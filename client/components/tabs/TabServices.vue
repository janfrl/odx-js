  ...
  <div class="h-full flex flex-col bg-base text-base font-sans">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        >
            ...
            <div class="min-w-0 flex-1">
              <div class="flex items-center gap-2 mb-1">
                ...
                <span
                  ...
                  :style="{ backgroundColor: generatingStatus[svc.name] ? '#94a3b8' : (svc.isGenerated ? COLORS.green : COLORS.orange) }"
                  :class="[
                    generatingStatus[svc.name] ? 'animate-pulse scale-125' : '',
                    svc.isGenerated ? 'shadow-[0_0_8px_rgba(0,220,130,0.3)]' : 'shadow-[0_0_8px_rgba(249,115,22,0.3)]'
                  ]"
                />
              </div>
          ...
          <div class="mt-6 pt-4 border-t border-base flex justify-between items-center text-[10px] uppercase font-bold tracking-widest relative z-10">
            ...
            <span v-if="generatingStatus[svc.name]" class="text-zinc-400 animate-pulse">Generating...</span>
            <span 
              v-else
    ...
    <div v-else class="h-full flex flex-col pt-4 overflow-hidden">
      <div class="px-6 flex items-center gap-4 mb-6 shrink-0 font-sans text-base">
        ...
        >
          <!-- Carbon: progress-bar-round (SVG Spinner) -->
          <svg v-if="generatingStatus[selectedService.name]" class="animate-spin h-3.5 w-3.5 text-current" viewBox="0 0 32 32">
            <path d="M16 4V2a14 14 0 000 28v-2a12 12 0 010-24z" fill="currentColor" opacity=".2"/>
            <path d="M16 4V2a14 14 0 00-14 14h2a12 12 0 0112-12z" fill="currentColor"/>
          </svg>
