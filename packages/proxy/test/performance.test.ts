...
import { clearODataLogs } from '@me-tools/odx-core'
...
async function measureSequential(label: string, path: string, request: () => Promise<unknown>): Promise<MeasurementSummary> {
  ...
  return {
    ...
    concurrency: 1,
    ...summarizeBenchmarkTimings(timings, roundAvgMs),
  }
}
...
describe('proxy benchmark timing summary math', () => {
  it('summarizes fixed request timings and round averages deterministically', () => {
    expect(summarizeBenchmarkTimings(
      [10, 2, 4, 8, 6],
      [8, 4, 10],
    )).toEqual({
      minMs: 2,
  ...
  it('returns zero summary values for empty timing fixtures', () => {
    expect(summarizeBenchmarkTimings([], [])).toEqual({
      minMs: 0,
      avgMs: 0,
      medianRoundAvgMs: 0,
      roundStdDevMs: 0,
