...
export function generateRegistryDts(
  ...
  const indexDtsLines = ['import type { ODataMergeService, ODataServiceRegistry } from "@me-tools/odx-core"']
  ...
  for (const [svcName, entities] of Object.entries(serviceEntities)) {
    ...
    const modelMapping = serviceModelFiles[svcName]
      ...
      : 'Record<string, any>'
    indexDtsLines.push(`    ${formatTypeKey(svcName)}: ODataMergeService<${entityUnion}, ${modelMapping}>`)
  }
  indexDtsLines.push('  }')
  indexDtsLines.push('}')
