...
export function generateRegistryDts(
  ...
  const indexDtsLines = ['import type { ODataServiceRegistry, ODataVersionedService } from "@me-tools/odx-core"']
  ...
  for (const [svcName, entities] of Object.entries(serviceEntities)) {
    ...
    const modelMapping = serviceModelFiles[svcName]
      ...
      : 'Record<string, any>'
    indexDtsLines.push(`    ${formatTypeKey(svcName)}: ODataVersionedService<${entityUnion}, ${modelMapping}>`)
  }
  indexDtsLines.push('  }')
  indexDtsLines.push('}')
