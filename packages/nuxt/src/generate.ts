...
export function generateRegistryDts(
  ...
  const indexDtsLines = ['import type { ODataConcurrencyService, ODataServiceRegistry } from "@me-tools/odx-core"']
  ...
  for (const [svcName, entities] of Object.entries(serviceEntities)) {
    ...
    const modelMapping = serviceModelFiles[svcName]
      ...
      : 'Record<string, any>'
    indexDtsLines.push(`    ${formatTypeKey(svcName)}: ODataConcurrencyService<${entityUnion}, ${modelMapping}>`)
  }
  indexDtsLines.push('  }')
  indexDtsLines.push('}')
